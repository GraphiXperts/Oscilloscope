...
AnalyzeWidget::AnalyzeWidget(QWidget *parent) :
    QWidget(parent),
    osc_ui(new Ui::AnalyzeWidget),
	fs_ui(new Ui::AnalyzeWidget)
{
    osc_ui->setupUi(this);
	fs_ui->setupUi(this);
    instance = this;

    osc_layout=osc_ui->verticalLayout;
	fs_layout=fs_ui->verticalLayout;
    ...
    osc_ui->actionSimultaneous_moving->setChecked(Config::multipleBias);
	fs_ui->actionSimultaneous_moving->setChecked(Config::multipleBias);
}
...
void AnalyzeWidget::analyze(std::shared_ptr<Graph2DData> data, QPointer<GraphTemplate> templ, glType t) {
    ...
    switch (t) {
        case glType::Oscillogram: {
            ...
            osc_layout->addWidget(gView);
            gView->resize(300, 100);
            gView->show();
...
void AnalyzeWidget::SetGlobalScale() {
    ...
    for (qint64 i = 0; i < osc_layout->count(); ++i) {
       auto osc = qobject_cast<glTemplateOscillogram*>( osc_layout->itemAt(i)->widget());
...
void AnalyzeWidget::on_actionSimultaneous_moving_triggered() {
    Config::multipleBias = osc_ui->actionSimultaneous_moving->isChecked();
    Config::multipleBias = fs_ui->actionSimultaneous_moving->isChecked();
}
